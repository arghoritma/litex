...
<header
  class="bg-white/80 dark:bg-gray-900 dark:border-b dark:border-gray-700 backdrop-blur-md fixed w-full z-50 shadow-sm"
...
>
    ...
    <div class="hidden md:flex lg:space-x-4">
      {#each menuLinks.filter((item) => item.show) as item}
        <a
          use:inertia
          href={item.href}
          class="nav-link dark:text-gray-200 dark:hover:dark:text-gray-100 dark:hover:bg-gray-800 {item.group ===
          group
  ...
  {#if isMenuOpen}
    ...
    <div
      ...
      class="fixed inset-0 bg-black/20 backdrop-blur-sm z-50 md:hidden {isMenuOpen
        ? 'block'
        : 'hidden'}"
      ...
      >
        <div class="flex flex-col p-4 space-y-4">
          {#each menuLinks.filter((item) => item.show) as item}
            <a
              href={item.href}
              class="mobile-nav-link dark:text-gray-200 dark:hover:bg-gray-700 dark:hover:text-white {item.group ===
              group
                ? 'active'
                : ''}"
            >
        ...
        <div class="px-4 py-3 border-t dark:border-gray-700 border-gray-200">
          <div class="flex items-center space-x-3">
            ...
            {:else}
              <a href="/login" class="flex-1 btn-secondary text-sm py-2"
                >Masuk</a
              >
              <a href="/register" class="flex-1 btn-primary text-sm py-2"
                >Daftar</a
              >
            {/if}
          </div>
